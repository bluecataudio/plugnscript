...
/**
 *  \file WaveFile.hxx
 *  Simple wave file utilities for c++ scripting.
 *
...
{
    uint	channelsCount=0;        // number of audio channels
                    ...
                    if(fmtSize>=16) // PCM format size is 16 or 18
                    {
                        ...
                        if(format==1 || format==3) //uncompressed PCM format or float are supported
                        {
                            ...
                            while(tempString!="data" && !f.isEndOfFile())
                            {
                                ...
                                f.movePos((int)dataSize);        // skip junk data
...
struct WaveFileData
{
    ...
    uint		  channelsCount;        ///< number of audio channels
    ...
    {
        if(channelsCount!=0)
            return int(interleavedSamples.length/channelsCount);
        else
            return 0;
    }
    ...
    bool loadFile(string filePath)
    {
        ...
        if(f.open(filePath,"r")>=0)
        {
            ...
            if(header.read(f))
            {
                ...
                interleavedSamples.length=uint(header.samplesCount*channelsCount);
...
struct WaveFileReader
{
     ...
     */
    bool openFile(const std::string& filePath, int maxChannelsCount=-1)
    {
        bool ok=false;
        if(f.open(filePath,"r")>=0)
        {
            if(header.read(f))
            {
                ...
                if(maxChannelsCount>0 && uint(maxChannelsCount)<header.channelsCount)
                {
                    ...
                    channelsToSkip=uint(header.channelsCount-maxChannelsCount);
                }
                else
                {
                    channelsToRead=uint(header.channelsCount);
                    channelsToSkip=0;
                }
            }
        }
     ...
     */
    void readSample(double oSample[])
    {
        switch(header.bytesPerSample)
        {
    ...
    {
        return int(header.channelsCount);
    }
    
    // private data
private:
    file    f; // the file
    uint    blockSize=0; // sample block size (all channels)
    uint    channelsToRead=0; // number of channels to read from file
    uint    channelsToSkip=0; // number of channels to skip (channels not used)
    double gainFactor=1; // gain factor for integer files
    WaveFileHeader header; // the wave file header data
};
...
struct WaveFileWriter
{
     ...
     */
    void writeSample(const double iSample[])
    {
    ...
    // data
    file    f; ///< the audio file.
    double  gainFactor=1; ///< factor used for integer wav files.
};
#endif
